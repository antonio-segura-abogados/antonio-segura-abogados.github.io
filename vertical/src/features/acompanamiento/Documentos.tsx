...
const guide: GuideStep[] = [
  { title: 'Una carpeta comprensible', text: 'Cada documento muestra su estado. Filtra para ver qué requiere tu atención.', target: 'archivos' },
  { title: 'Corregir con instrucciones', text: 'Abre el certificado de nacimiento. La observación indica qué falta y permite aportar una copia de ejemplo.', target: 'archivos' },
  { title: 'Recibir y revisar', text: 'La copia nueva queda en revisión. La validación sigue siendo una tarea del equipo.' },
];
...
export function DocumentosScene() {
  ...
  return <><DemoScene title="Mis documentos" label="Tu carpeta documental" guide={guide} navigation activeSection="documents" note="Archivos ficticios. Esta lista ilustra estados, no requisitos de un trámite." onReset={() => { setDocs(data.documentos); setSelected(null); setFilter('todos'); }}>{highlight => <>
