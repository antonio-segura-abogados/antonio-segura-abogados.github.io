...
import qrDestinations from '../../../../recursos-compartidos/qr/destinos.json';
import logo from '../../../../recursos-compartidos/assets/marca/logo-azul-web.jpg';
import { presentationLink } from './enlaces';

const total = book.slides.length;
const qrUrls = new Set([...qrDestinations.routes.map(item => item.qrUrl), ...qrDestinations.external.map(item => item.url)]);

function SlideImage({ slide }: { slide: typeof book.slides[number] }) {
  const qrUrl = slide.enlaces.find(url => qrUrls.has(url));
  const image = <img className="book-current-slide" src={asset(slide.imagen)} width={slide.ancho} height={slide.alto} alt={`Página ${slide.numero}. ${slide.titulo}`} fetchPriority="high" />;
  return qrUrl ? <a className="book-slide-link" href={presentationLink(qrUrl)} target="_blank" rel="noopener noreferrer" aria-label={`Abrir el enlace QR de la página ${slide.numero}: ${slide.titulo} (nueva pestaña)`} title="Abrir el enlace del QR en una nueva pestaña">{image}</a> : image;
}
...
export function Presentacion({ expanded = false }: { expanded?: boolean }) {
  ...
  useEffect(() => {
    function keyboard(event: KeyboardEvent) {
      ...
      if (showIndex || (zoom && stage.current?.contains(target)) || target.closest('input,select,textarea,[contenteditable=true]') || event.altKey || event.ctrlKey || event.metaKey) return;
  ...
  useEffect(() => {
    ...
    setReadText(false);
    window.scrollTo(0, 0);
    if (expanded) heading.current?.focus({ preventScroll: true });
  }, [expanded]);
  useEffect(() => { stage.current?.scrollTo(0, 0); }, [index, zoom]);
  useEffect(() => {
  ...
  return <main className={`presentation${expanded ? ' presentation-expanded' : ''}`} id="contenido">
    ...
    <section className="book-reader" aria-label="Presentación completa">
      <div className="book-reader-top">
        {expanded ? <Link className="book-return" to={`/?pagina=${slide.numero}`}><Arrow back />Volver al inicio</Link> : <span>LA PRESENTACIÓN <i />{total} PÁGINAS</span>}
        <div>
          <button onClick={() => setShowIndex(true)}><svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true"><path d="M1 1h5v5H1zm9 0h5v5h-5zM1 10h5v5H1zm9 0h5v5h-5z" fill="none" stroke="currentColor" /></svg>Ver índice</button>
          {expanded ? <button disabled={readText} aria-pressed={zoom} onClick={() => setZoom(value => !value)}>{zoom ? 'Ajustar página' : 'Acercar ×2'}</button> : <Link to={`/presentacion?pagina=${slide.numero}`}><svg viewBox="0 0 18 18" width="16" height="16" aria-hidden="true"><path d="M2 7V2h5m4 0h5v5M2 11v5h5m4 0h5v-5" fill="none" stroke="currentColor" strokeWidth="1.3" /></svg>Ampliar</Link>}
        </div>
      </div>
      <div className={`book-stage${expanded && zoom && !readText ? ' is-zoomed' : ''}`} ref={stage} tabIndex={expanded ? 0 : undefined} role={expanded ? 'region' : undefined} aria-label={expanded ? 'Página de la presentación; usa las flechas para cambiar de página' : undefined}>
        {expanded && readText ? <section id="texto-pagina" className="book-transcript"><h2>Página {slide.numero} · {slide.titulo}</h2><p>{slide.texto}</p></section> : <figure className="book-paper"><SlideImage key={slide.imagen} slide={slide} /></figure>}
      </div>
      <div className="book-reading-controls"><div className="book-current-title"><span>{slide.capitulo}</span><h1 ref={heading} tabIndex={-1}>{slide.titulo}</h1></div><div className="book-pagination" aria-label="Cambiar de página"><button aria-label="Página anterior" disabled={index === 0} onClick={() => go(index - 1)}><Arrow back /></button><span><b>{String(slide.numero).padStart(2,'0')}</b><i>/</i>{String(total).padStart(2,'0')}</span><button aria-label="Página siguiente" disabled={index === total - 1} onClick={() => go(index + 1)}><Arrow /></button></div><div className="book-page-action">{demo ? <Link to={demo.split('#')[1]}>Probar esta idea <Arrow /></Link> : source ? <a href={source} target="_blank" rel="noreferrer">{sourceLabel}<span aria-hidden="true">↗</span></a> : <span className="book-keyboard-hint">← → para pasar página</span>}</div></div>
      <div className="book-progress" aria-hidden="true"><span style={{ width: `${(index + 1) / total * 100}%` }} /></div>
      {!expanded && <div className="book-thumbnails" ref={thumbs} aria-label="Miniaturas de las páginas">{book.slides.map((item, i) => <button key={item.numero} aria-label={`Ir a página ${item.numero}: ${item.titulo}`} aria-current={i === index ? 'page' : undefined} onClick={() => go(i)}><img src={asset(item.miniatura)} alt="" width="150" height="106" loading="lazy" /><span>{String(item.numero).padStart(2,'0')}</span></button>)}</div>}
      ...
      {readText && !expanded && <section id="texto-pagina" className="book-transcript"><h2>Página {slide.numero} · {slide.titulo}</h2><p>{slide.texto}</p></section>}
    ...
    {!expanded && <footer className="book-footer"><span>Pol Surriel · Una candidatura con una propuesta concreta.</span><Link to="/demos">Continuar con las demostraciones <Arrow /></Link></footer>}
  </main>;
}
