...
const routeGuide: GuideStep[] = [
  { title: 'La ruta se configura', text: 'Selecciona un paso y cambia sus instrucciones. Los tipos de paso son funciones ya construidas: documentos, formularios, revisiones y seguimiento.', target: 'editor' },
  { title: 'Vista del cliente', text: 'La vista previa usa los mismos datos que el editor. Prueba a mover o añadir un paso. Aún estás trabajando en un borrador.' },
  { title: 'Revisión antes de publicar', text: 'Simula la revisión profesional y publica la versión 2. Cualquier edición posterior a la revisión la invalida. Los expedientes abiertos conservan su versión.' },
];
const changeGuide: GuideStep[] = [
  { title: 'Entender qué cambia', text: 'Compara las instrucciones antes y después de un cambio ficticio. Una persona determina el alcance y la fecha de vigencia.', target: 'cambio' },
  { title: 'Decidir sobre cada expediente', text: 'La versión nueva no cambia los casos abiertos por sí sola. Aplica el ejemplo a Lucía y comprueba que conserva su avance. Omar requiere revisión individual.' },
  { title: 'Una excepción acotada', text: 'Añade una comprobación solo a Lucía. La plantilla general y el otro expediente permanecen iguales.' },
];
...
export function RutasScene({ catalogo = false }: { catalogo?: boolean }) {
  ...
  return <DemoScene title="Configurar las rutas" label="Operación · rutas de acompañamiento" guide={routeGuide} workspace onReset={reset} note="Gestión simulada. El criterio jurídico y la publicación requieren supervisión profesional.">{highlight => <>
    ...
    {!opened ? <div className="route-catalog"><div><span className="status-label">VERSIÓN 1 · PUBLICADA</span><h2>{data.ruta.nombre}</h2><p>5 pasos · instrucciones, documentos y revisiones</p><button className="primary-button" onClick={() => setOpened(true)}>Abrir configurador<Icon name="arrow" /></button></div><p className="quiet-note">Ejemplo de catálogo. Cada tipología tendría su propia ruta revisada.</p></div> : <>
      <div className={`management-board ${highlight === 'editor' ? 'is-highlighted' : ''}`} data-capture="board">
...
export function CambiosScene({ expediente = false }: { expediente?: boolean }) {
  ...
  return <><DemoScene title="Cambios y casos particulares" label="Operación · versiones y alcance" guide={changeGuide} workspace onReset={reset} note="Cambio de procedimiento ficticio. No reproduce una reforma legal ni interpreta el BOE.">{highlight => <>
    ...
    <div className={`changes-board ${highlight === 'cambio' ? 'is-highlighted' : ''}`} data-capture="board">
