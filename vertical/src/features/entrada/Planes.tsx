...
export function PlanesScene({ checkout = false }: { checkout?: boolean }) {
  ...
  return <><DemoScene dialog={
    <SceneDialog open={!!chosen} onClose={() => setChosen(null)} title="Resumen del plan">{chosen && <Checkout plan={chosen} period={period} />}</SceneDialog>
  } title={checkout ? 'Contratación de ejemplo' : 'Elige tu acompañamiento'} label="Acompañamiento a tu medida" onReset={() => { setPeriod('mensual'); setChosen(null); setResetKey(k => k + 1); }} note={data.precioNota}>{checkout ? <Checkout key={resetKey} plan={data.planes[1]} period="mensual" /> : <>
    <div className="greeting"><p>Elige cómo te acompañamos.</p><h1>Un plan para avanzar.</h1></div>
    <div className="billing-toggle" aria-label="Periodicidad">{(['mensual', 'anual'] as const).map(value => <button key={value} aria-pressed={period === value} className={period === value ? 'selected' : ''} onClick={() => setPeriod(value)}>{value === 'mensual' ? 'Mensual' : 'Anual · ahorra 2 meses'}</button>)}</div>
    <div className="plan-list">{data.planes.map((plan, i) => <section className={`plan-card ${i === 1 ? 'plan-card--plus' : ''}`} key={plan.id}>
